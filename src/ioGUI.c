...
void afficheBool(int b){
	if (b == 1) fprintf(stderr, "Oui");
	else if (b == 0) fprintf(stderr, "Non");
	else fprintf(stderr, "afficheBool : ERREUR");
}
...
void debut_jeu(grille *g, grille *gc){
  ...
  int scr;
	...
	g->cyclique = 0;
	fprintf(stderr, "Mode cyclique désactivé\n");
	g->vieillissement = 0;
	fprintf(stderr, "Vieillissement des cellules non pris en compte\n");
			...
			case KeyRelease:
				...
				switch(buff)
				{
					case 'c':
						...
						fprintf(stderr, "Mode cyclique : ");
						afficheBool(g->cyclique);
					break;
					case 'v':
						g->vieillissement = !g->vieillissement;
						fprintf(stderr, "Prise en compte du vieillissement : ");
